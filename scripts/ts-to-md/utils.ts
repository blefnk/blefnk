 ...
 * @returns Array of projects with the specified tag
 */
export function getProjectsByTech(
	projects: readonly Project[],
	tech: TechStack,
): Project[] {
	return projects.filter((project) =>
		project.technologies.some((t) => t === tech),
	);
}
...
export async function getProjectsWithNoReleases(
	...
	const { projectsWithValidRepos, repoData } =
		await getValidRepositoryProjects(projects);
	...
	console.log(
		`Found ${finalResults.length} projects with no release information.`,
	);
...
export async function getProjectsWithReleasesBeforeDate(
	...
	if (!parsedDate) {
		throw new Error(
			`Unable to parse date: ${dateString}. Please provide a valid date format.`,
		);
	}

	console.log(
		`Finding projects with releases before ${parsedDate.toISOString().split("T")[0]}...`,
	);
	...
	let finalResults = [
		...projectsWithEarlierReleases,
		...projectsWithNoReleaseDate,
	];
...
export async function getProjectsWithReleasesBetweenDates(
	...
	if (!startDate || !endDate) {
		throw new Error(
			"Unable to parse one or both dates. Please provide valid date formats.",
		);
	}
	...
	for (const project of filteredProjects) {
		...
		if (repoDataEntry?.lastUpdated) {
			if (
				repoDataEntry.lastUpdated >= startDate &&
				repoDataEntry.lastUpdated <= endDate
			) {
				result.push(project);
			}
		}
	...
	console.log(
		`Found ${finalResults.length} projects with releases between the specified dates.`,
	);
 ...
 */
export function normalizeWhoUses(
	whoUses: WhoUses,
): ReadonlyArray<FeaturedUser> {
	...
	for (const [index, project] of projects.entries()) {
		for (const key of requiredKeys) {
			if (project[key] === undefined || project[key] === null) {
				errors.push(
					`Project[${index}] (id: ${project.id}) is missing required key: ${key}`,
				);
			}
		}
 ...
 */
async function getValidRepositoryProjects(
	projects: readonly Project[],
): Promise<{
...
function parseFlexibleDateFormat(dateString: string): Date | null {
	...
	const dynamicRegex =
		/^(\d+)\s+(day|days|week|weeks|month|months|year|years)\s+(ago|from now)$/i;
