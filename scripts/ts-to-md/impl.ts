import { projects } from "scripts/ts-to-md/projects";
...
export async function generateMarkdownTable(
	...
	const validCategoryKeys = selectedCategoryKeys.filter(
		(catKey) => AVAILABLE_CATEGORIES[catKey],
	);
			...
			for (const project of group.projects) {
				...
				if (repoInfo) {
					...
					if (
						!reposToFetch.some(
							(r) =>
								r.apiPath === repoInfo.apiPath &&
								r.platform === repoInfo.platform,
						)
					) {
	...
	const finalCategories = processedCategories.filter(
...
function generateTable(
	...
	const maxRows = Math.max(
		...categories.map((group) => group.projects.length),
		0,
	);
	...
	const rows = Array.from({ length: maxRows }, (_, rowIndex) =>
		categories
			.map((group) =>
				projectToMarkdownCell(group.projects[rowIndex], options, repoData),
			)
			.join(" | "),
	);
...
function sortProjects(
	...
	return [...projectsToSort].sort((a, b) => {
		...
		const repoDataA: RepoData | undefined = repoInfoA
			? repoData[repoInfoA.apiPath]
			: undefined;
		const repoDataB: RepoData | undefined = repoInfoB
			? repoData[repoInfoB.apiPath]
			: undefined;
