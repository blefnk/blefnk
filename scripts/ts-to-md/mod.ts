...
import { AVAILABLE_CATEGORIES, DEFAULT_RETRIES_NUMBER } from "./constants";
import { generateMarkdownTable } from "./impl";
...
async function dateQueryExamples(projects: readonly Project[]) {
	...
	const beforeProjects = await getProjectsWithReleasesBeforeDate(
		projects,
		"01-01-2025",
	);
	console.log(
	...
	const oldOssProjects = await getProjectsWithReleasesBeforeDate(
		projects,
		"6 months ago",
		{
			ossOnly: true,
	...
	console.log(
		`Found ${last3MonthsProjects.length} projects released in the last 3 months`,
	);
	...
	console.log(
		`Found ${projectsWithNoReleaseInfo.length} OSS projects with no release information`,
	);
...
if (EXAMPLE_TO_RUN < 1 || EXAMPLE_TO_RUN > 10) {
	console.error(
		"Invalid example number selected. Please choose between 1 and 10.",
	);
} else {
...
async function exampleFunctions() {
	...
	console.log(
		`${getProjectsByStatus(projects, "ACTIVE").length} active projects`,
	);

	console.log("👇 getProjectsByCategory");
	console.log(
		`${getProjectsByCategory(projects, "library").length} library projects`,
	);

	console.log("👇 getProjectsByTag");
	console.log(
		`${getProjectsByTag(projects, "cli").length} projects with 'cli' tag`,
	);

	console.log("👇 getProjectsByTech");
	console.log(
		`${getProjectsByTech(projects, "Polar").length} projects with 'Polar' tech`,
	);
