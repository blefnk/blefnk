...
export const apiCache = (() => {
	...
	function set<T>(
		key: string,
		data: T,
		ttlSeconds = CACHE_CONFIG.DEFAULT_TTL,
	): void {
		const now = Date.now();
...
export async function fetchAllRepoData(
	...
	const [githubData, gitlabData, bitbucketData] = await Promise.all([
		...
		gitlabRepos.length > 0
			? fetchGitLabRepoData(gitlabRepos, options)
			: Promise.resolve({}),
	...
	console.log(
		`Repository data fetching complete in ${fetchEndTime - fetchStartTime}ms.`,
	);
		...
		await processBatch(
			...
			async (repo) => {
				try {
					...
					console.warn(
						`Failed to fetch Bitbucket data for ${repo.apiPath}:`,
						error,
					);
				}
			},
			...
			async (repo) => {
				try {
					...
					if (error instanceof Error && !error.message.includes("not found")) {
						console.warn(
							`Failed to fetch GitHub version for ${repo.apiPath}:`,
							error,
						);
					}
				}
		...
		await processBatch(
			...
			async (repo) => {
				try {
					...
					console.warn(
						`Failed to fetch GitHub repo data for ${repo.apiPath}:`,
						error,
					);
				}
			},
...
export async function fetchGitHubRepoDataWithUngh(
		...
		await processBatch(
			...
			async (repo) => {
				try {
					...
					if (data.release?.tag) {
						if (!repoData[repo.apiPath]) repoData[repo.apiPath] = {};
						repoData[repo.apiPath].version = data.release.tag.replace(
							/^v/i,
							"",
						);

						if (data.release.publishedAt) {
							repoData[repo.apiPath].lastUpdated = new Date(
					...
					if (error instanceof Error && !error.message.includes("not found")) {
						console.warn(
							`Failed to fetch release info for ${repo.apiPath} with UNGH:`,
							error,
						);
					}
				}
	...
	if (
		options.sortBy === "updated" ||
		(options.sortBy === "stars" && !options.stars)
	) {
			...
			await processBatch(
				...
				async (repo) => {
					try {
						...
						console.warn(
							`Failed to fetch repo data for ${repo.apiPath} with UNGH:`,
							error,
						);
					}
				},
		...
		await processBatch(
			...
			async (repo) => {
				try {
					...
					if (
						(options.stars || options.sortBy === "stars") &&
						data.star_count !== undefined
					) {
					...
					console.warn(
						`Failed to fetch GitLab data for ${repo.apiPath}:`,
						error,
					);
				}
			},
...
export async function fetchWithRetry<T>(
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 403) {
					...
					console.warn(
						`Rate limited (403). Reset in ${Math.ceil(waitTime / 1000)}s`,
					);
	...
	throw (
		lastError || new Error(`Failed to fetch ${url} after multiple attempts`)
	);
}
...
export function getRequestConfig(options: GenerateOptions): RequestConfig {
	return {
		cacheDuration:
			options.cacheDuration ?? DEFAULT_REQUEST_CONFIG.cacheDuration,
			...
			batch.map(async (item) => {
				...
				if (processed % 5 === 0 || processed === total) {
					...
					process.stdout.write(
						`Processing items: ${processed}/${total} (${Math.round(
							(processed / total) * 100,
						)}%)`,
					);
				}
